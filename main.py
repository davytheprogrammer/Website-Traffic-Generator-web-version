...
# Check arguments for their validity
if args.threads < 1:
        ...
        try:
            ...
            for i in range(random.randint(args.min_clicks, args.max_clicks + 1)):
                try:
                    ...
                    print("[{}] Found {} links".format(time.strftime("%H:%M:%S", time.gmtime()), len(links)))
                    # If there are no links, we go back.
                    if len(links) == 0:
                        print("[{}] Visiting now: {}".format(time.strftime("%H:%M:%S", time.gmtime()), "previous page"))
                        driver.back()
                        continue
                    # Ok, there is a new link to click. Visit it.
                    next_link = random.choice(links)
                    print("[{}] Visiting now: {}".format(time.strftime("%H:%M:%S", time.gmtime()), next_link.get_attribute("href")))
